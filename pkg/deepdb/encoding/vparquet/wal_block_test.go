...
func TestPartialReplay(t *testing.T) {
	...
	// Flush a set of snapshots across 2 pages
	count := 10
	ids := make([]common.ID, count)
	trs := make([]*deeptp.Snapshot, count)
	for i := 0; i < count; i++ {
		trs[i] = test.GenerateSnapshot(i, nil)
		ids[i] = trs[i].ID

		b1, err := decoder.PrepareForWrite(trs[i], 0)
		require.NoError(t, err)

		b2, err := decoder.ToObject(b1)
		require.NoError(t, err)

		err = w.Append(ids[i], b2, 0)
		require.NoError(t, err)
	...
	for ; ; gotCount++ {
		...
		// Find snapshot in the input data
		match := 0
		for i := range ids {
...
func TestWalBlockFindSnapshotByID(t *testing.T) {
	testWalBlock(t, func(w *walBlock, ids []common.ID, trs []*deeptp.Snapshot) {
		for i := range ids {
			found, err := w.FindSnapshotByID(context.Background(), ids[i], common.DefaultSearchOptions())
			require.NoError(t, err)
...
// of walblock. it also ignores the passed in snapshots and ids and simply asserts that the row iterator
// is internally consistent.
func TestRowIterator(t *testing.T) {
	testWalBlock(t, func(w *walBlock, _ []common.ID, _ []*deeptp.Snapshot) {
...
func testWalBlock(t *testing.T, f func(w *walBlock, ids []common.ID, trs []*deeptp.Snapshot)) {
