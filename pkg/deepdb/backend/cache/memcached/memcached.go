...
func NewClient(cfg *Config, cfgBackground *cache.BackgroundConfig, logger log.Logger) cache.Cache {
	...
	client := cache.NewMemcachedClient(cfg.ClientConfig, "deep", prometheus.DefaultRegisterer, logger)
	...
	c := cache.NewMemcached(memcachedCfg, client, "deep", prometheus.DefaultRegisterer, logger)
