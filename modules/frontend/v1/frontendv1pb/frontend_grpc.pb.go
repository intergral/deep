...
type FrontendClient interface {
	...
	ProcessTracepoint(ctx context.Context, opts ...grpc.CallOption) (Frontend_ProcessTracepointClient, error)
...
func (c *frontendClient) ProcessTracepoint(ctx context.Context, opts ...grpc.CallOption) (Frontend_ProcessTracepointClient, error) {
	stream, err := c.cc.NewStream(ctx, &Frontend_ServiceDesc.Streams[1], "/frontend.Frontend/ProcessTracepoint", opts...)
	if err != nil {
		return nil, err
	}
	x := &frontendProcessTracepointClient{stream}
	return x, nil
}

type Frontend_ProcessTracepointClient interface {
	Send(*ClientToFrontend) error
	Recv() (*FrontendToClient, error)
	grpc.ClientStream
}

type frontendProcessTracepointClient struct {
	grpc.ClientStream
}

func (x *frontendProcessTracepointClient) Send(m *ClientToFrontend) error {
	return x.ClientStream.SendMsg(m)
}

func (x *frontendProcessTracepointClient) Recv() (*FrontendToClient, error) {
	m := new(FrontendToClient)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type FrontendServer interface {
	...
	// a "FrontendToClient" message and replies with single "ClientToFrontend" message.
	// This version differs from the above as this is called from the tracepoint service
	ProcessTracepoint(Frontend_ProcessTracepointServer) error
	// The client notifies the query-frontend that it started a graceful shutdown.
...
func (UnimplementedFrontendServer) ProcessTracepoint(Frontend_ProcessTracepointServer) error {
	return status.Errorf(codes.Unimplemented, "method ProcessTracepoint not implemented")
}
...
func _Frontend_ProcessTracepoint_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FrontendServer).ProcessTracepoint(&frontendProcessTracepointServer{stream})
}

type Frontend_ProcessTracepointServer interface {
	Send(*FrontendToClient) error
	Recv() (*ClientToFrontend, error)
	grpc.ServerStream
}

type frontendProcessTracepointServer struct {
	grpc.ServerStream
}

func (x *frontendProcessTracepointServer) Send(m *FrontendToClient) error {
	return x.ServerStream.SendMsg(m)
}

func (x *frontendProcessTracepointServer) Recv() (*ClientToFrontend, error) {
	m := new(ClientToFrontend)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var Frontend_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ProcessTracepoint",
			Handler:       _Frontend_ProcessTracepoint_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
	Metadata: "frontend/v1/frontendv1pb/frontend.proto",
}
