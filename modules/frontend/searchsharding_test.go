...
// implements deepdb.Reader interface
type mockReader struct {
	metas []*backend.BlockMeta
}

func (m *mockReader) FindSnapshot(ctx context.Context, tenantID string, id common.ID, blockStart string, blockEnd string, timeStart int64, timeEnd int64) (*deeptp.Snapshot, []error, error) {
	return nil, nil, nil
}
...
func (m *mockReader) Fetch(ctx context.Context, meta *backend.BlockMeta, req deepql.FetchSnapshotRequest, opts common.SearchOptions) (deepql.FetchSnapshotResponse, error) {
...
func TestIngesterRequest(t *testing.T) {
	timeNow := time.Now()
	now := int(timeNow.Unix())
	tenMinutesAgo := int(timeNow.Add(-10 * time.Minute).Unix())
	fifteenMinutesAgo := int(timeNow.Add(-15 * time.Minute).Unix())
	twentyMinutesAgo := int(timeNow.Add(-20 * time.Minute).Unix())
	...
	tests := []struct {
		...
		{
			name:                "start/end is outside queryIngestersUntil",
		...
		{
			name:                "start/end is inside queryBackendAfter",
		...
		{
			name:                "backendAfter/ingsetersUntil = 0 results in no ingester query",
			request:             "/?tags=foo%3Dbar&minDuration=10ms&maxDuration=30ms&limit=50&start=" + strconv.Itoa(tenMinutesAgo) + "&end=" + strconv.Itoa(now),
		...
		{
			name:                "ingester start/End = 15 - 10 mins ago",
		...
		{
			name:                "backend start/End = 15 - 10 mins ago",
			request:             "/?tags=foo%3Dbar&minDuration=10ms&maxDuration=30ms&limit=50&start=" + strconv.Itoa(tenMinutesAgo) + "&end=" + strconv.Itoa(now),
			queryIngestersUntil: 15 * time.Minute,
			expectedURI:         "/querier?end=" + strconv.Itoa(now) + "&limit=50&maxDuration=30ms&minDuration=10ms&start=" + strconv.Itoa(tenMinutesAgo) + "&tags=foo%3Dbar",
		},
		...
		{
			name:                "backend start/End = 20 - 5 mins ago",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedResponse != nil {
				...
				if tc.expectedResponse.Snapshots != nil {
					assert.Equal(t, len(tc.expectedResponse.Snapshots), len(actualResp.Snapshots))
					assert.Equal(t, tc.expectedResponse.Snapshots[0].SnapshotID, actualResp.Snapshots[0].SnapshotID)
				} else {
					assert.Equal(t, tc.expectedResponse.Snapshots, actualResp.Snapshots)
				}
				assert.Equal(t, tc.expectedResponse.Metrics.InspectedSnapshots, actualResp.Metrics.InspectedSnapshots)
				assert.Equal(t, tc.expectedResponse.Metrics.InspectedBlocks, actualResp.Metrics.InspectedBlocks)
				assert.Equal(t, tc.expectedResponse.Metrics.InspectedBytes, actualResp.Metrics.InspectedBytes)
				assert.Equal(t, tc.expectedResponse.Metrics.SkippedBlocks, actualResp.Metrics.SkippedBlocks)
				assert.Equal(t, tc.expectedResponse.Metrics.SkippedSnapshots, actualResp.Metrics.SkippedSnapshots)
				assert.Equal(t, tc.expectedResponse.Metrics.TotalBlockBytes, actualResp.Metrics.TotalBlockBytes)
			}
		})
	}
}
