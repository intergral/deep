...
import (
	...
	"github.com/intergral/deep/modules/distributor/receiver"
...
func New(cfg Config, clientCfg ingester_client.Config, ingestersRing ring.ReadRing, generatorClientCfg generator_client.Config, generatorsRing ring.ReadRing, o *overrides.Overrides, middleware receiver.Middleware, logger log.Logger, loggingLevel logging.Level, reg prometheus.Registerer) (*Distributor, error) {
	...
	if o.IngestionRateStrategy() == overrides.GlobalIngestionRateStrategy {
		...
		ring, err := ring.New(lifecyclerCfg.RingConfig, "distributor", cfg.OverrideRingKey, logger, prometheus.WrapRegistererWithPrefix("deep_", reg))
		if err != nil {
...
func (d *Distributor) PushTraces(ctx context.Context, traces ptrace.Traces) (*deeppb.PushResponse, error) {
	...
	// deeppb.Trace is wire-compatible with ExportTraceServiceRequest
...
func (d *Distributor) sendToIngestersViaBytes(ctx context.Context, userID string, traces []*rebatchedTrace, keys []uint32) error {
	...
	err := ring.DoBatch(ctx, op, d.ingestersRing, keys, func(ingester ring.InstanceDesc, indexes []int) error {
		...
		req := deeppb.PushBytesRequest{
			Traces:     make([]deeppb.PreallocBytes, len(indexes)),
			Ids:        make([]deeppb.PreallocBytes, len(indexes)),
		...
		_, err = c.(deeppb.PusherClient).PushBytesV2(localCtx, &req)
...
// requestsByTraceID takes an incoming deepdb.PushRequest and creates a set of keys for the hash ring
	...
	for _, b := range batches {
		...
		for _, ils := range b.ScopeSpans {
			for _, span := range ils.Spans {
				...
				traceKey := deep_util.TokenFor(userID, traceID)
				ilsKey := traceKey
				if ils.Scope != nil {
